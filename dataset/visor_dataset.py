...
class VISORDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Open meta file and read relations
        with open(os.path.join(os.path.dirname(im_root), 'meta.json'), 'r') as f:
            video_data = json.load(f)['videos']
        self.relations = {}
        for vid in video_data:
            self.relations[vid] = {k : v.get('relations', []) for k, v in video_data[vid]['frames'].items()}
    ...
    def __getitem__(self, idx):
        ...
        vid_relations = self.relations[video]
        info = {}
        info['name'] = video
        ...
        while trials < 5:
            ...
            # Step 1: Randomly choose 3 frames with some determined distance
            info['frames'] = []
            ...
            masks = []
            boundaries = []
            relations = vid_relations[frames[frames_idx[0]][:-4]] # relation from first frames
            tmp = {}
            for pair in relations:
                tmp.setdefault(pair[0], []).append(pair[1])
            relations = tmp
            ...
            # Step 3: Select 2 objects over it
            """
            Logic note:
            ...
            """
            try_again = ((self.include_hand and len(labels[labels <= 2]) == 0)
            ...
            if try_again:
                ...
                if self.include_hand:
                    # Require at least one hand in the first frame
                    hand_labels = labels[labels <= 2]
                    target_object = np.random.choice(hand_labels) # hand object
                    # Get hand-held object from here
                    if relations and target_object in relations:
                        has_second_object = True
                        empty_cb = False
                        second_object = np.random.choice(relations[target_object])
                    else:
                        # no relation -> choose random object, can be hand other another hand's in contact object
                        has_second_object = len(labels[labels != target_object]) > 0
                        empty_cb = True
                        if has_second_object:
                            # that object must not in contact with hand
                            second_object = np.random.choice(labels[labels != target_object])

                else:
                    # Like YouTubeVOS, choose random two objects but not hands
        ...
        boundary_masks = np.stack(boundaries, axis=0).astype(np.float32) # (3, 2, H, W)
        # _tmp = boundary_masks.copy()

        if has_second_object:
            selector = torch.FloatTensor([1, 1])
            second_masks = (masks == second_object).astype(np.float32)[:, np.newaxis, :, :]
            if self.include_hand:
                boundary_masks = boundary_masks[:, target_object - 1, :, :]
                if empty_cb:
                    boundary_masks.fill(0)
            info['second_object'] = second_object
        ...
        info['target_object'] = target_object
        ...
        data = {
            ...
            'selector': selector,           # (2, )                 -> torch.tensor
            'info': info
        }
...
#         self.relations = {}
#         for vid in video_data:
#             self.relations[vid] = {k + '.jpg' : v.get('relations', None) for k, v in video_data[vid]['frames'].items()}
...
#                     # Only get from the first frame
#                     relations = vid_relations[jpg_name]
...
#                     # Require at least one hand in the first frame
#                     hand_labels = labels[labels <= 2]
#                     target_object = np.random.choice(hand_labels)
#                     # Get hand-held object from here
